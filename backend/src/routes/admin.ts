...
// ── Middleware: verifica token de plataforma ──────────────────────────────────
...
// Lista todas as empresas com contagem de usuários e módulos ativos
router.get('/tenants', adminAuth, async (_req: Request, res: Response): Promise<void> => {
  const result = await query(`
    SELECT
      ...
      COUNT(DISTINCT tm.module_id) AS active_module_count,
...
// Detalhes de uma empresa: usuários + módulos ativos
router.get('/tenants/:id', adminAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const [tenantRes, usersRes, modulesRes] = await Promise.all([
    ...
    query(
      'SELECT id, email, full_name, role, is_active, last_login_at, created_at FROM user_profiles WHERE tenant_id = $1 ORDER BY created_at',
      [id]
    ),
    query(
      'SELECT module_id, is_active, activated_at FROM tenant_modules WHERE tenant_id = $1 ORDER BY activated_at',
      [id]
    ),
  ]);
...
// ── POST /api/admin/seed ──────────────────────────────────────────────────────
// Cria o primeiro super admin (só funciona se não existir nenhum)
router.post('/seed',
